...
def _flash_script_path(config: str) -> str:
    return f".zed/flash-{config}.cmake"


def _cmake_arg(value) -> str:
    """CMake bracket argument: preserves spaces and backslashes verbatim."""
    return f"[[{value}]]"


def build_flash_script(ctx, config: str) -> str:
    """CMake script that runs build/convert/upload without shell operators.

    Zed launches tasks through the configured shell. In particular, Windows
    PowerShell 5.1 rejects `&&` even when it appears in arguments intended for
    `cmd.exe`. execute_process gives us portable sequencing and exit checking.
    """
    preset = _preset(ctx, config)
    flash = ctx.profile.flash_task(ctx.tc, _elf(ctx, config))
    steps = flash.get("pre_commands", []) + [{
        "command": flash["command"],
        "args": flash.get("args", []),
    }]

    lines = [
        "# Generated by stm32-init; rerun the generator instead of editing.",
        "cmake_minimum_required(VERSION 3.22)",
        "",
        "function(run_checked description)",
        "  execute_process(",
        "    COMMAND ${ARGN}",
        f"    WORKING_DIRECTORY {_cmake_arg(ctx.project_dir)}",
        "    RESULT_VARIABLE step_result",
        "    COMMAND_ECHO STDOUT",
        "  )",
        "  if(NOT step_result EQUAL 0)",
        '    message(FATAL_ERROR "${description} failed with exit code ${step_result}")',
        "  endif()",
        "endfunction()",
        "",
        "if(BUILD_FIRST)",
        "  run_checked(\"firmware build\"",
        f"    {_cmake_arg(ctx.tc.cmake)} --build --preset {_cmake_arg(preset)}",
        "  )",
        "endif()",
        "",
    ]
    for index, step in enumerate(steps, start=1):
        command = " ".join(
            [_cmake_arg(step["command"])]
            + [_cmake_arg(arg) for arg in step.get("args", [])]
        )
        lines += [
            f'run_checked("flash step {index}"',
            f"  {command}",
            ")",
            "",
        ]
...
def build_tasks(ctx):
    ...
    for config in ("Debug", "Release"):
        ...
        flash_label = flash.get("label", "Flash")
        flash_script = _flash_script_path(config)
        ...
        task(f"{flash_label} ({config})", tc.cmake,
             ["-DBUILD_FIRST=OFF", "-P", flash_script])
        # Zed has no dependsOn. The CMake helper sequences steps without
        # relying on shell-specific operators such as &&.
        task(f"Build + {flash_label} ({config})", tc.cmake,
             ["-DBUILD_FIRST=ON", "-P", flash_script], save="all")
...
class ZedBackend(EditorBackend):
    ...
    def generate(self, ctx):
        ...
        for config in ("Debug", "Release"):
            write_text(ctx.project_dir / _flash_script_path(config),
                       build_flash_script(ctx, config), ctx.dry_run)
    ...
    def next_steps(self, ctx):
        ...
        flash_label = ctx.profile.flash_task(ctx.tc, _elf(ctx, "Debug")).get("label", "Flash")
        steps = [
            ...
            f"'{prefix}Build + {flash_label} (Debug)' builds and flashes in one task.",
            "clangd picks up "
